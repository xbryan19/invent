...
<v-container fluid>
        ...
        <v-card-text>
            <v-row dense>
                <v-col cols="12" md="3" sm="4">
                    <v-select v-model="insert_data.req" outlined dense label="REQUEST KIND" target="#dropdown-example" item-text="BarchartName" item-value="BarchartNo" :items="reqKind" disabled></v-select>
                </v-col>

                <v-col cols="12" md="2" sm="4">
                    <v-text-field v-model="insert_data.week" label="WEEK" dense outlined></v-text-field>
                </v-col>

                <v-col cols="12" md="2" sm="4">
                    <v-text-field v-model="insert_data.year" label="YEAR" dense outlined></v-text-field>
                </v-col>

                <v-col cols="12" md="3" sm="4">
                    <v-select v-model="insert_data.dstn" outlined dense label="DSTN" target="#dropdown-example" item-text="DSTN" :items="dstn" disabled></v-select>
                </v-col>

                <v-col cols="12" md="2" sm="4">
                    <v-btn dark color="primary" @click="filter_expand()">
    ...
    <v-row>
        <v-col cols="12" lg="6" md="6" sm="12">
            <v-card outlined>
                <v-card-title><small>LIST OF PLANS</small>
                </v-card-title>
                <v-card-text>
...
export default {
    ...
    methods: {
        selected_counter(){
            return this.filteredExpand.filter(each=>{return each.check == true})
        },
        unSelectAll() {
        ...
        // checkB(customer) {
        //     var indexNum = this.filteredExpand.findIndex(obj => obj.codeNo==customer)
        //     var collectedData = this.filteredExpand[indexNum].HINBAN

        //     if (this.filteredExpand[indexNum].check == true) {
        //         // this.codex.push(this.filteredExpand[indexNum].codeNo)
        //         this.codez = this.filteredExpand[indexNum].codeNo
        //         console.log(this.codez, 'codez')
        //         axios.post(`${this.url}checkbox`, {
        //         // pasa: this.grupo,
        //         customerCode: this.codez
        //         })
        //         .then(res=>{
        //             console.log(res.data[0].HINBAN, 'response')
        //             var hinban = res.data[0].HINBAN.length
        //             for (var x = 0; x < hinban; x++) {
        //                 if (typeof res.data[0].HINBAN[x].childData === 'undefined') {
        //                     console.log('1')
        //                     var children = res.data[0].HINBAN[x].childData.length
        //                     for (var y = 0; y < children; y++) {
        //                         this.subti.push({
        //                             child: res.data[0].HINBAN[x].childData[y].HinbanPartCode,
        //                             qty: res.data[0].HINBAN[x].childData[y].Quantity,
        //                             onStock: res.data[0].HINBAN[x].childData[y].OnStockQty,
        //                             availableQty: res.data[0].HINBAN[x].childData[y].AvailableQty,
        //                             motherQty: res.data[0].HINBAN[x].qtyData,
        //                             year: res.data[0].HINBAN[x].year,
        //                             week: res.data[0].HINBAN[x].week,
        //                             codeNo: res.data[0].codeNo,
        //                             barchartNo: res.data[0].HINBAN[x].BarchartNo,
        //                             houseCode: res.data[0].HOUSECD,
        //                             motherHinban: res.data[0].HINBAN[x].hinbanData
        //                         })
        //                     }
        //                 }
        //             }
        //             console.log(this.subti, 'subti')
        //             // this.checkBox()
                    
        //         })
        //     } else {
        //         // this.codex.splice(indexNum, 1)
        //         this.codez = ''
        //     }
            
        //     // var codex = this.filteredExpand[indexNum].codeNo
        //     // this.values = []

        //     // if (this.filteredExpand[indexNum].check == true) {
        //     //     for (var x = 0; x < collectedData.length; x++) {
        //     //         this.grupo.push(collectedData[x].hinbanData)
        //     //     }
        //     // } else {
        //     //     this.grupo.splice(indexNum, 1)
        //     // }
        // },
        ...
        // checkAvailability(cCode) {
        //     this.availability.customerCode = cCode
        //     console.log(this.availability)
        //     axios.post(`${this.url}checkboxParts`, {
